(async function(){
  ...
  MMS.bindAttr('#galleryVideo', 'src', content.hero?.video_src || '');
  ...
  if (dbWrap){
    dbWrap.innerHTML = db.map(x => `<div class="fact benefit"><span class="dot">•</span><strong>${x}</strong></div>`).join('');
  }
  ...
  // Gallery
  let g = content.gallery?.images || [];
  const galleryData = await loadJSON('assets/data/gallery.json');
  if (galleryData && Array.isArray(galleryData.images) && galleryData.images.length){
    g = galleryData.images;
  }
  const gWrap = document.querySelector('#galleryGrid');
  if (gWrap){
    gWrap.innerHTML = g.map(i => `<div class="gimg"><img src="${i.src}" alt="${i.alt || ''}" loading="lazy"></div>`).join('');
  }
  ...
  if (faqWrap && content.faq?.items){
    faqWrap.innerHTML = content.faq.items.map(x => `
      <details class="faq-item">
        <summary>
          <span class="faq-question">${x.q}</span>
          <span class="faq-icon" aria-hidden="true"></span>
        </summary>
        <div class="faq-answer">${x.a_html ?? x.a ?? ''}</div>
      </details>
    `).join('');
  }
